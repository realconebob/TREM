...
func AddReminder(file string, args []string) error {
	...
	currentReminders, err := misc.GetFromGobFile[reminders.Entry](file)
	...
	return misc.SerializeToGobFile(currentReminders, file, reminders.GetReminderGobFilter())
}
...
func ListReminders(file string, args []string) error {
	reminders, err := misc.GetFromGobFile[reminders.Entry](file)
	if err != nil {
		return err
	}
